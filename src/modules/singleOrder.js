const fetchSingleOrder = (id) => (
  fetch(`https://18d9382a-0730-4340-952c-340f90890c88.mock.pstmn.io/orders/${id}`, {
    headers: {'x-api-key': 'f747cdbe08f3497395174d140b3fa1f4'}
  })
...
const singleOrder = (state = {}, action) => {
  const index = {
    ...
    'FETCH_SUCCESS': () => (
      {...state, singleOrder: action.payload, isLoading: false, error: null}
    ),
    'FETCH_ERROR': () => ({...state, singleOrder: null, isLoading: false, error: true}),
    'DEFAULT': () => (state),
  };
